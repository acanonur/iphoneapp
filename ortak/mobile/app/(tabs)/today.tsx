...
import { useStore, selectAll, memberColor, memberName, pendingCount } from '../../src/store/useStore.js';
...
export default function TodayScreen() {
  ...
  const todays = useMemo(
    () =>
      events
        .filter((e) => e.endsAt >= now && e.startsAt <= now + 2 * DAY)
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['left', 'right']}>
      ...
      >
        ...
        >
          {todays.length === 0 ? (
            <Muted>Nothing in the next two days.</Muted>
          ) : (
            todays.map((event) => (
              <Card key={event.id} onPress={() => router.push('/(tabs)/calendar')}>
                <Row style={{ justifyContent: 'space-between' }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Muted>
                      ...
                      {event.allDay ? ' · all day' : ` · ${clockTime(event.startsAt)}`}
        ...
        <Section title="To do" actionLabel="All lists" onAction={() => router.push('/(tabs)/lists')}>
          {openTasks.length === 0 ? (
